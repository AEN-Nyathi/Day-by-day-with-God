...
export default function grade2PossibleSums(numberOfQuestions: number): Question[] {
    ...
    for (let i = 0; i < numberOfQuestions; i++) {
        ...
        do {
            num1 = Math.floor(Math.random() * 10); // Generate random number between 0 and 100
            num2 = Math.floor(Math.random() * 10); // Generate random number between 0 and 100
            sum = num1 + num2;
        } while (
            sum > 20 ||
            questions.some(
        ...
        questions.push({
            ...
            question: `${num1} + ${num2} = `,
            answer: sum,
            AvelableAnswers: [Math.floor(Math.random() * 10), sum - 3, sum - 2, sum - 1, sum, sum + 1, sum + 2, sum + 3, Math.floor(Math.random() * 10)].sort(() => Math.random() - 0.5)
